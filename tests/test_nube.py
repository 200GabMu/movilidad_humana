...
# ── 5b) el caso de Streamlit Cloud: el disco arranca con la copia VIEJA de main
#        (solo v2) mientras GitHub ya tiene v2 + v3. Al arrancar, GitHub manda y
#        NADA se borra; la versión nueva reaparece en el disco.
v3 = almacen.guardar_version(df_crudo=df, df_limpio=df, modo=almacen.MODO_AGREGAR, archivos=[], hojas=1, nota="v3", stats={}, usuario="t")
assert nube.sincronizar("v3").ok
import json  # noqa: E402
hist = json.loads(almacen.ARCHIVO_HISTORIAL.read_text(encoding="utf-8"))
hist["versiones"] = [v for v in hist["versiones"] if v["id"] != v3.id]; hist["activa"] = v2.id
almacen.ARCHIVO_HISTORIAL.write_text(json.dumps(hist), encoding="utf-8")
shutil.rmtree(almacen.DIR_VERSIONES / v3.id)            # disco "viejo": sin v3
r = nube.sincronizar_al_arrancar()
assert r is not None and r.ok, r
ids = {v.id for v in almacen.listar_versiones()}
assert v3.id in ids and v2.id in ids and almacen.id_activa() == v3.id, (ids, almacen.id_activa())
assert any(p.startswith(f"data/versiones/{v3.id}/") for p in remoto), "v3 sigue en GitHub"
r = nube.sincronizar("tras arrancar")
assert r.ok and not r.borrados, r.resumen()
print("arranque con disco viejo: GitHub manda y no se borra nada OK")

# ── 5c) una versión que solo está en el disco (subida a mano en main) se conserva y sube
v4 = almacen.guardar_version(df_crudo=df, df_limpio=df, modo=almacen.MODO_AGREGAR, archivos=[], hojas=1, nota="solo local", stats={}, usuario="t")
r = nube.sincronizar_al_arrancar()
assert r is not None and r.ok, r
assert any(p.startswith(f"data/versiones/{v4.id}/") for p in remoto) and v4.id in {v.id for v in almacen.listar_versiones()}
print("versión solo local se conserva y se sube OK")

# ── 5d) eliminar desde el Historial sí borra en GitHub
almacen.eliminar_version(v4.id)
r = nube.sincronizar("borrar v4")
assert r.ok and any(v4.id in b for b in r.borrados) and not any(v4.id in p for p in remoto)
print("eliminar desde Historial borra en GitHub OK")
