...
def sincronizar(mensaje: str = "Actualización de datos desde la app", progreso=None) -> ResultadoSync:
    ...
    # Solo se borra en GitHub lo que el usuario eliminó desde el Historial:
    # archivos de versiones cuyo id ya no figura en historial.json. Cualquier
    # otro archivo remoto que falte en disco se deja (nunca se pierde nada por
    # una descarga incompleta o un disco desactualizado).
    ids_locales = _ids_historial_local()
    for ruta_repo, sha in remoto.items():
        if ruta_repo in locales:
            continue
        partes = ruta_repo.split("/")
        es_version = len(partes) > 3 and partes[1] == "versiones"
        if not es_version or partes[2] in ids_locales:
            continue
        try:
...
def _ids_historial_local() -> set[str]:
    try:
        data = json.loads(almacen.ARCHIVO_HISTORIAL.read_text(encoding="utf-8"))
        return {v["id"] for v in data.get("versiones", [])}
...
def restaurar_si_vacio() -> ResultadoSync | None:
    """Compatibilidad: ahora el arranque siempre combina con GitHub."""
    return sincronizar_al_arrancar()


def sincronizar_al_arrancar() -> ResultadoSync | None:
    """Al arrancar la app (p. ej. tras un reinicio de Streamlit Cloud, donde el
    disco vuelve a tener la carpeta data/ antigua del repositorio):

      1. Si en la rama de datos hay algo, se descarga TODO: GitHub es la
         verdad, porque ahí quedó lo último que se unificó.
      2. Las versiones que solo existan en el disco (p. ej. las que se subieron
         a mano en main) se conservan y se añaden al historial.
      3. Se sube a GitHub lo que falte, sin borrar nada.

    Devuelve None si no hay configuración."""
    cfg = config()
    if cfg is None:
        return None
    # historial local antes de tocar nada
    local: dict = {}
    try:
        local = json.loads(almacen.ARCHIVO_HISTORIAL.read_text(encoding="utf-8"))
    except (OSError, ValueError):
        local = {}
    try:
        remoto = _arbol_remoto(cfg)
    except NubeError as exc:
        return ResultadoSync(errores=[str(exc)])
    if not remoto:
        # primera vez: no hay nada en GitHub todavía; lo del disco se sube
        return sincronizar("Primera copia de data/ en GitHub") if local else ResultadoSync()

    res = restaurar()
    if res.errores:
        return res
    # combinar: versiones locales que GitHub no tiene se conservan
    try:
        nuevo = json.loads(almacen.ARCHIVO_HISTORIAL.read_text(encoding="utf-8"))
    except (OSError, ValueError):
        return res
    ids_remotos = {v["id"] for v in nuevo.get("versiones", [])}
    extra = [v for v in local.get("versiones", []) if v["id"] not in ids_remotos
             and (almacen.DIR_VERSIONES / v["id"] / "limpio.parquet").exists()]
    if extra:
        nuevo["versiones"] = sorted(nuevo.get("versiones", []) + extra, key=lambda v: v["id"])
        if not nuevo.get("activa") or not (almacen.DIR_VERSIONES / str(nuevo["activa"]) / "limpio.parquet").exists():
            nuevo["activa"] = nuevo["versiones"][-1]["id"]
        almacen.ARCHIVO_HISTORIAL.write_text(json.dumps(nuevo, ensure_ascii=False, indent=2), encoding="utf-8")
        subida = sincronizar("Versiones del disco añadidas a la copia")
        res.subidos += subida.subidos
        res.errores += subida.errores
    return res
