...
_COLUMNAS_KIT_OTROS = ("OTROS (SE DEBE ESPECIFICAR",)


def limpiar_valores_fuera_de_lugar(df: pd.DataFrame) -> tuple[pd.DataFrame, int]:
    """Arregla dos errores de digitación frecuentes en los reportes:
      * un correo escrito en una columna que no es de correo (p. ej. en
        CONDICIÓN SOLO PARA NNA) -> se vacía, para que no aparezca como
        categoría en los gráficos;
      * en OTROS (kit humanitario), 'ALIMENTACIÓN' / 'ALMENTOS' -> 'ALIMENTOS'.
    Devuelve (df, celdas corregidas)."""
    df = df.copy()
    n = 0
    for col in df.columns:
        c_up = str(col).upper()
        if "CORREO" in c_up or "EMAIL" in c_up or col in COLUMNAS_SIN_MAYUSCULAS:
            continue
        es_correo = df[col].map(lambda x: isinstance(x, str) and bool(_RE_CORREO.match(x.strip())))
        if es_correo.any():
            df.loc[es_correo, col] = None
            n += int(es_correo.sum())
        if c_up.startswith(_COLUMNAS_KIT_OTROS):
            es_alimento = df[col].map(lambda x: isinstance(x, str) and re.search(r"AL[I]?MENT", x.upper()) is not None
                                      and x.strip().upper() != "ALIMENTOS")
            if es_alimento.any():
                df.loc[es_alimento, col] = "ALIMENTOS"
                n += int(es_alimento.sum())
    return df, n
...
def procesar_limpieza_y_relleno(df: pd.DataFrame, corregir_encabezados: bool = True) -> tuple[pd.DataFrame, dict]:
    ...
    df, n_fuera = limpiar_valores_fuera_de_lugar(df)
    ...
    stats = {
        ...
        "valores_fuera_de_lugar": n_fuera,
