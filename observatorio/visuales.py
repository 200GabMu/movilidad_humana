...
def _limpio(nombre: str | None) -> str:
    """Nombre de campo legible para un título: sin el texto entre paréntesis
    del formulario y sin espacios dobles."""
    if not nombre:
        return ""
    import re
    n = re.sub(r"\s*\(.*?\)\s*", " ", str(nombre)).strip()
    n = re.sub(r"\s+", " ", n)
    return {"OTROS": "KIT DE ALIMENTOS (OTROS)"}.get(n.upper(), n.upper())


def titulo_de(v: Visual) -> str:
    """Todo gráfico lleva título: el del Power BI si lo tiene; si no, se arma
    con los campos que muestra (así ninguna gráfica queda sin nombre)."""
    if v.titulo:
        return v.titulo
    if v.tipo == "matriz":
        return f"{_limpio(v.cat)} POR {_limpio(v.columnas)}"
    if v.cat and v.serie:
        return f"{_limpio(v.cat)} POR {_limpio(v.serie)}"
    if v.cat:
        return _limpio(v.cat)
    return _limpio(v.etiqueta)


def _titulo(v: Visual) -> None:
    t = titulo_de(v)
    if t:
        st.markdown(f'<div class="pbi-titulo">{t}</div>', unsafe_allow_html=True)
...
NOTAS_CAMPO = {
    ...
    "CONDICIÓN SOLO PARA NNA": (
        "<b>NNA</b> = niños, niñas y adolescentes (0 – 17 años) &nbsp;·&nbsp; "
        "<b>ACOMPAÑADO</b> con su madre, padre o tutor legal &nbsp;·&nbsp; "
        "<b>NO ACOMPAÑADO</b> sin ningún adulto responsable &nbsp;·&nbsp; "
        "<b>SEPARADO</b> sin madre ni padre, pero con otro familiar o adulto a cargo &nbsp;·&nbsp; "
        "<b>No aplica</b> personas adultas"
    ),
    "EMBARAZO": (
        "Solo aplica a mujeres. Cada barra se divide en <b>SI</b> (amarillo) y <b>NO</b> (azul); "
        "el número es la cantidad de personas de cada parte."
    ),
    "ACTUALMENTE ESTÁ ESTUDIANDO": "<b>SI</b> / <b>NO</b> según lo declarado en el reporte mensual.",
}
